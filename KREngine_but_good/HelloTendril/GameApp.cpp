...
namespace
{
	...
	std::vector<Math::Matrix4> GetWorldTransforms(std::vector<Bone*> bones)
	{
		...
		{
			bones[i]->transform = bones[i]->transform;
...
void GameApp::OnInitialize(uint32_t width, uint32_t height)
{
	...
	mBoneConstantBuffer.Initialize();
	mSkinningVertexShader.Initialize(L"../Assets/Shaders/Skinning.fx", Graphics::VertexBone::Format);
	mSkinningPixelShader.Initialize(L"../Assets/Shaders/Skinning.fx");
}
...
void GameApp::OnTerminate()
{
	...
	//Terminate
	mSkinningVertexShader.Terminate();
	mSkinningPixelShader.Terminate();
...
void GameApp::OnUpdate()
{
	...
	worldMatrix = Math::Matrix4::Translation(-5.0f, 0.0f, 0.0f);
	...
	{
		// render tendril
		Math::Matrix4 boneTransforms[64];
		for (size_t i = 0; i < mBoneWorldTransforms.size(); ++i)
		{
			boneTransforms[i] = mBoneWorldTransforms[i];
		}
		mBoneConstantBuffer.Set(boneTransforms);
		mBoneConstantBuffer.BindVS(1);
		mSkinningVertexShader.Bind();
		mSkinningPixelShader.Bind();
	}
