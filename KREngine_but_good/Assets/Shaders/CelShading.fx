...
float4 PS(VSOutput input) : SV_Target
{
    ...
    float4 diffuseCel = GradientMap.Sample(SamplerPoint, float2(diffuse, 0.0f));
    float4 diffuseColor = diffuseCel * lightDiffuse * materialDiffuse;
    
	//Specular
    float3 halfVector = normalize((dirToLight + dirToCamera) * 0.5f);
    float specular = saturate(dot(halfVector, normal));
    float specularCel = GradientMap.Sample(SamplerPoint, float2(specular, 0.0f));
    float4 specularColor = pow(specularCel, power) * lightSpecular * materialSpecular;
